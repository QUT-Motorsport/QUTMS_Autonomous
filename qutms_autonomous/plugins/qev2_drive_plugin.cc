...
namespace gazebo
{
...

    {
        ...
        public: virtual void Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
        {
            ...
            // Create two subscribers to two topics
            ros::SubscribeOptions so = ros::SubscribeOptions::create<std_msgs::Float32>("/" + this->model->GetName() + "/vel_cmd", 1, boost::bind(&qev2Plugin::OnRosVelMsg, this, _1), ros::VoidPtr(), &this->ros_queue); 
            ros::SubscribeOptions sop = ros::SubscribeOptions::create<std_msgs::Float64>("/" + this->model->GetName() + "/turn_cmd", 1, boost::bind(&qev2Plugin::OnRosTurnMsg, this, _1), ros::VoidPtr(), &this->ros_queue);
	        this->ros_subv = this->ros_node->subscribe(so);
	        this->ros_subt = this->ros_node->subscribe(sop);
        ...
        public: void SetTurn(const double &_turn) {
            // Set the joint turn
            this->model->GetJointController()->SetVelocityTarget(this->joint->GetScopedName(), _turn);
            this->model->GetJointController()->SetVelocityTarget(this->joint2->GetScopedName(), _turn);            
        }
        ...
        public: void OnRosTurnMsg(const std_msgs::Float64ConstPtr &_msg) {
            this->SetTurn(_msg->data);
        }
        ...
        // Subscriber
        private: ros::Subscriber ros_subv;
        private: ros::Subscriber ros_subt;
